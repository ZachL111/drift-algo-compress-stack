...
`drift-algo-compress-stack` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for compress analysis with transition tables, invalid-transition tests, and documented operating limits.
...
The score is deterministic. The current threshold is 173. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 5.
